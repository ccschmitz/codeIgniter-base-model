    ...
    function add($options = array())
    {
        ...
        foreach ($this->fields as $field) {
            if (isset($options[$field])) {
                if (in_array($field, $this->json)) {
                    $options[$field] = json_encode($options[$field]);
                }
                if (in_array($field, $this->set) && is_array($options[$field])) {
                    if (is_array($options[$field])) {
                        $options[$field] = join(',', $options[$field]);
                    }
                }
    ...
    function get($options = array())
    {
        ...
        if (isset($options[$this->primary_key])) {
            ...
            foreach ($row as $field => $value) {
                ...
                if (in_array($field, $this->set)) {
                    $row[$field] = explode(',', $value);
                }
            }
            return $row;
        } else {
